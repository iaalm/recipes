...
int X = 100,Y = 100,wide = 4,space = 1,loops = 300;			//长 宽 边长 间距
char *a,*b;
...
gint run(GtkWidget *widget)
{
	...
	if(times == loops || times == 0)			//过多次数后，重新开始
	{
		times = 0;
		for(i = 0;i < X;i++)
			for(j = 0;j < Y;j++)
			{
				...
				cairo_rectangle (cr, i * wide, j * wide, (wide -space), (wide-space));	//格子之间留一些空间
				cairo_fill (cr);
			}
	}
	else
		...
		char *argv[])
{
	...
	gtk_widget_set_size_request (da, X * wide, Y * wide);
